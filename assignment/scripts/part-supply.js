...
console.log('New Array is now:', supplyChanges);
...
let sum = 0;
for (let c = 0; c < supplyChanges.length; c++) {
  sum += supplyChanges[c]
}
...
let totalParts = 572;
let boxCapacity = 7;
let boxesFilled = totalParts % boxCapacity;
let newBox = totalParts - 7;

do {
    (totalParts > boxCapacity ) 
    totalParts--;
    console.log('Keep adding parts until full', totalParts)}
    while (totalParts > boxCapacity );
console.log( "Total Boxes Filled:", boxesFilled );
